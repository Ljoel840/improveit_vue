        ...
        <div class="tituloNombre" :style="margen">
          <h1>{{atleta.titulo}}</h1>
			...
			<div class="contenedorResto container-fluid">
				<img id="show-modal" @click="showModal = true; urlImg=atleta.imagenAlternativa.url" :src="atleta.imagenAlternativa.url" v-if="atleta.imagenAlternativa">
				<img id="show-modal" @click="showModal = true; urlImg=atleta.imagenAlternativa2.url" :src="atleta.imagenAlternativa2.url" v-if="atleta.imagenAlternativa2">
				<span v-for="imag in atleta.imagenes" :key="imag.id" style="padding:0">
					<img id="show-modal" @click="showModal=true; urlImg=imag" :src="imag" v-if="imag">
				</span>
...
export default {
  ...
  data() {
    return {
	  atleta: [],
	  urlImg:'',
	  atletaSeleccionado: this.$route.params.atletaSeleccionado,
	  ocultar: false,
	  parrafos: [],
	  HTMLcontent: null,
      ancho: {
  ...
  created () {
	...
	this.atletaSeleccionado=this.atletaSeleccionado.replace(/_/g, " ")
	let datos = axios.post(`${process.env.API_CONTACT_URL}dfs60024`,{
        ...
        .then(response => {
			...
			for (var i = 0; i < this.deportistas.length; i++){
			if(this.deportistas[i].titulo === this.atletaSeleccionado){
				this.atleta = this.deportistas[i]
				console.log ('esto:'+this.atleta.imagen.url)
				break
			}
			}
        })
